function [result, result_c0,result_c1,result_cresto] = charge_exploration_filtered(file)
...
%threshold: an intensity threshold to filter signals
...
for n = 1:n_s
    ...
    display(['The maximum m/z intensity in this sample is: ', num2str(max_intensity)])
    threshold = input('Introduce an intensity threshold (usually between 1-0.1% of the maximum intensity): ');

    m(s_f,:) = [];
    intens = find(m(:,2) < threshold);
    m(intens,:) = [];
    result{n,1} = m;
